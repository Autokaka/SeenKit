...
ModulePtr CreateModule(const fs::path& path) {
  ...
  SEEN_INFO("Create wasm_module on worker: {}.", worker_name);
  std::ifstream ifs(path, std::ios::binary);
  CFDeferredTask defer_ifs([&ifs]() { ifs.close(); });
  if (!ifs) {
    SEEN_ERROR("Failed to create wasm_module on worker: {}. File not exists!", worker_name);
    return nullptr;
  }
  ...
  if (!ifs) {
    SEEN_ERROR("Failed to create wasm_module on worker: {}. Failed to write bytes!", worker_name);
    return nullptr;
  }
  if (!wasm_module_validate(store.get(), &bytes)) {
    SEEN_ERROR("Failed to create wasm_module on worker: {}. Invalid module!", worker_name);
    return nullptr;
  }
